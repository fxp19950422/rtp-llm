...
class IndexerFP8PrepareVarlenTest(unittest.TestCase):
    ...
    def test_b1_varlen_kwargs_bit_equal_to_legacy(self) -> None:
        """B==1 with full varlen kwargs (``use_varlen=True``) must produce
        bit-equal meta to the legacy scalar-only call (``use_varlen=False``).
        Guards against accidental divergence in the new branch."""
    ...
    def test_b2_ke_clamps_per_request_T_b(self) -> None:
        ...
        T_b (NOT global T_total) so a future regression that accidentally
        clamped against T_total wouldn't truncate req1's window. Bit-exact
        contract because ``IndexerFP8.forward`` subtracts
        ``cu_kv_per_token`` to convert back to per-request-local before
        feeding ``combine_topk_swa_indices``."""
        kw = self._make_batched_kwargs(
            prefix_lengths=[0, 8],
            input_lengths=[16, 12],
        )
        ...
        ke_cpu = meta.ke.cpu()
        # cu_kv_seqlens = [0, 4, 9] → req0 offset=0, req1 offset=4.
        # Req0 last token (pos=15, b=0): ke = 0 + clamp((15+1)//4=4, max=4) = 4
        self.assertEqual(int(ke_cpu[15].item()), 4)
        # Req1 last token (pos=19, b=1): ke = 4 + clamp((19+1)//4=5, max=5) = 9
        self.assertEqual(int(ke_cpu[27].item()), 9)
        # First req0 token (pos=0, b=0): ke = 0 + clamp(1//4=0, max=4) = 0
        self.assertEqual(int(ke_cpu[0].item()), 0)
        # First req1 token (pos=8, b=1): ke = 4 + clamp(9//4=2, max=5) = 6
        self.assertEqual(int(ke_cpu[16].item()), 6)
