...
Returns a call-local fp32 accumulator. Eager storage is released with the
output; CUDA graph allocations are retained by PyTorch's graph memory pool.
...
from .fp8_fp4_base import Fp8Fp4ExecutorBase, split_moe_w13_gate_up
...
def _has_local_loop_kernel() -> bool:
    """Whether the dense FP8/FP4 fallback is runnable on this host."""
    try:
        import deep_gemm
    except Exception:
        return False
    if not callable(getattr(deep_gemm, "fp8_fp4_gemm_nt", None)):
        return False
...
class LocalLoopExecutor(Fp8Fp4ExecutorBase):
    ...
    def setup_weights(self, layer_weights: Dict) -> None:
        ...
        layout = cfg.moe_w1_layout
        gate_w, up_w = split_moe_w13_gate_up(w13, inter, layout)
        gate_s, up_s = split_moe_w13_gate_up(s13, inter, layout)
        stacked_routed = {
            "w1_w": gate_w,
            "w1_s": gate_s,
    ...
    def _forward_into_buf(
        ...
        """Allocate a call-local accumulator and dispatch to eager / graph compute."""
        T = x.size(0)
        capturing = torch.cuda.is_current_stream_capturing()
        # Do not retain a prefill-sized tensor per layer. The caching allocator
        # reuses eager storage, and graph pools keep captured addresses alive
        # without executor-owned references surviving graph destruction.
        y = torch.zeros((T, self.cfg.dim), dtype=torch.float32, device=x.device)
        if capturing:
