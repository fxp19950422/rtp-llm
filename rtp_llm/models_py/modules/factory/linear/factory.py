...
class LinearFactory:
    ...
    def create_merged_linear(
        ...
        # Check INT8 per-channel (compressed-tensors format)
        # These weights are in standard [out, in] layout (non-transposed),
        # so merging gate+up must concatenate along dim 0 (out_features).
        use_int8_per_channel = False
        first_weight = weights.get(weight_keys[0])
        if first_weight is not None and first_weight.dtype == torch.int8:
            qm = getattr(quant_config, "get_method", None)
            if qm and qm() == "INT8_PER_CHANNEL_COMPRESSED":
                use_int8_per_channel = True

        # For INT8 per-channel [out, in] format, merge along dim 0 (out_features)
        # instead of dim=-1 which is designed for transposed [in, out] format.
        weight_merge_dim = 0 if use_int8_per_channel else dim

        # Merge weights
        weight_tensors = [weights[key] for key in weight_keys]
        merged_weight = torch.cat(weight_tensors, dim=weight_merge_dim)

        # Merge scales if needed (for FP8, NVFP4, and INT8 per-channel)
        merged_scales = None
        ...
        elif use_int8_per_channel and scale_keys:
            scale_tensors = [weights[key] for key in scale_keys if key in weights]
            if scale_tensors:
                # Per-channel scales are 1D or 2D [N] or [1, N]; concat along dim 0
