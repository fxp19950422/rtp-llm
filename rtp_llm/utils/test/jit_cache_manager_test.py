...
class BackendTest(JitCacheTestBase):
    ...
    def test_fake_gang_failed_rank_is_not_reported_ready(self):
        configs = self.make_configs(remote="/r")
        configs.distribute_config.fake_gang_env = True
        proc = mock.Mock(name="rank-0", pid=123)
        proc.is_alive.return_value = False
        with mock.patch.object(
            backend, "_create_rank_processes", side_effect=self._fake_create(proc)
        ), mock.patch.object(
            backend, "_wait_for_ranks_startup", side_effect=RuntimeError("rank failed")
        ), mock.patch.object(
            backend, "_send_pipe_status"
        ) as status, mock.patch.object(backend, "ProcessManager") as manager:
            with self.assertRaisesRegex(Exception, "rank failed"):
                backend.multi_rank_start(None, configs, pipe_writer=object())
        self.assertEqual(status.call_args[0][1], "failed")
        manager.assert_not_called()
