...
absl::Status MtpExecutor::dispatchDecodeAsync(const StreamGroups&                          stream_groups,
    ...
    if (useDeviceInput() && kv_cache_block_id.defined()
        ...
        // Non-LINEAR groups keep their page-table entries through specUpdate.
        // Publish an owned copy as well, so the scheduler can overlap the host
        // commit. Allocation/compaction boundaries still join bookkeeping and
        // invalidate the snapshot in GenerateStateMachine::handleRunning.
        if (!is_linear_attention_model_
            || hasLinearCacheSnapshotCapacity(
